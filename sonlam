-- Sơn Lâm Hub - MM2 Key System with Custom TikTok Link (Optimized: Instant Snap & Absolute Hit)
...
local AimCircle = Create("Frame", ScreenGui, {AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, Position = UDim2.new(0.5, 0, 0.5, 0), Size = UDim2.new(0, FOVRadius * 2, 0, FOVRadius * 2), Visible = false})
...
local function GetRole(p)
    ...
    if p.Character:FindFirstChild("Knife") or (p.Backpack and p.Backpack:FindFirstChild("Knife")) then 
        return Color3.fromRGB(255, 0, 0), "Murderer" 
    end
...
-- Hàm tìm kiếm mục tiêu tối ưu tuyệt đối (Bắt sát nút, quét chuẩn xác cả trước và sau khi click)
local function GetBestTarget()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP and p.Character then
            ...
            local bodyPart = p.Character:FindFirstChild("HumanoidRootPart") or p.Character:FindFirstChild("Torso")

            if (role == "Murderer" or p.Character:FindFirstChild("Knife") or (p.Backpack and p.Backpack:FindFirstChild("Knife"))) and hum and hum.Health > 0 and bodyPart then
                ...
                if onScreen then
                    ...
                    dist = 99999 -- Ưu tiên các mục tiêu gần trong tầm nhìn trước, sau đó quét phía sau
                end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aimbot Sheriff Render Lock (Bám tâm nhẹ nhàng trong vòng FOV 75)
    if _G.AimbotSheriff then
        pcall(function()
            ...
            if targetPart then
                local predictedPos = targetPart.Position + (targetPart.AssemblyLinearVelocity * 0.06)
...
-- Sự kiện bấm nút BẮN (Sửa tốc độ đón đầu + Khóa cứng ngay lập tức khi nhấp để triệt tiêu trượt đạn)
